...
const FaqsRC = (props) => {
  ...
  // handle See Faq Questions
  const handleSeeFaqQuestions = async (partId) => {
    await axios
      .post(
      ...
      .then((resp) => {
        if (resp?.status === 200 && resp?.data.status === "1") {
          setFaqQuestions(resp?.data.data.questions);
  ...
  return (
    <section className="w-100 h-100">
      ...
      {/* End Swiper Slider */}
      <div className="m-0 mb-3 mt-auto p-0 d-flex w-100 flex-row justify-content-center align-items-center">
        <div
          onClick={() => slidingSwiper("Next")}
          className={
            (counter === faq?.length ? "d-none" : "") +
        ...
        {hasExercise ? (
          <div
            onClick={() => handleSeeFaqQuestions(id)}
            className={
              (counter === faq?.length ? "d-flex" : "d-none") +
